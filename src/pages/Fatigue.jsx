...
export default function Fatigue() {
  ...
  function handleNext() {
    navigate('/thanks', { state: { ...state, fatigue } })
  }

  const options = t('fatigueOptions')
  ...
  return (
    <div className="bg-app relative overflow-hidden flex flex-col min-h-[100dvh]">
      ...
      <div className="flex flex-col flex-1 items-center justify-center fade-in">
        ...
        <h1 className="text-white font-extrabold text-[21px] text-center mb-1">{t('fatigueQuestion')}</h1>
        <p className="text-white/70 text-[13px] text-center mb-8">{t('fatigueSub')}</p>
        ...
        <div className="flex flex-col gap-3 w-full max-w-[300px] mb-10">
          {options.map(opt => (
            <button key={opt.value} onClick={() => setFatigue(fatigue === opt.value ? null : opt.value)}
              ...
              style={{
                background: fatigue === opt.value ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.22)',
                color:      fatigue === opt.value ? '#FF8040' : 'white',
                borderColor: fatigue === opt.value ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.4)',
                transform:  fatigue === opt.value ? 'scale(1.04)' : 'scale(1)',
              }}>
              {opt.emoji} {opt.label}
            </button>
          ))}
        ...
        <button onClick={handleNext}
          ...
          {fatigue !== null ? t('continueBtn') : t('skipBtn')}
        </button>
      </div>
      </div>
    </div>
